...
class MujocoFairino5EnvBase(MujocoEnvBase):
    ...
    def setup_robot(self, init_qpos):
        ...
        self.body_config_list = [
            ArmConfig(
                ...
                arm_root_pose=self.get_body_pose("fairino5_v6_root_frame"),
                ik_eef_joint_id=6,
    ...
    def _get_obs(self):
        ...
        # right_pinch and left_pinch are coupled by an equality constraint (see
        # fairino5_v6_config.xml), so right_pinch alone captures the gripper's 1-DoF
        # state. Reported directly in meters (its native slide-joint unit), matching
        # gripper_actuator's ctrlrange of [-0.015, 0.015] so that action_space bounds
        # (used e.g. by GraspPhaseBase.set_target_open/close) stay consistent with
        # the observed joint position.
        gripper_joint_pos = np.array([self.data.joint("right_pinch").qpos[0]])
        gripper_joint_vel = np.array([self.data.joint("right_pinch").qvel[0]])
